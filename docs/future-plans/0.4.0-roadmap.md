...
This document captures improvements planned for the 0.4.0 milestone. Both items were
identified as the highest-priority work after 0.3.0: one is a correctness fix (ADR-005)
that unlocks the core multi-user deployment scenario, and one is a write throughput
improvement (A+B) that makes the post-ADR-005 throughput ceiling competitive with
the target use cases documented in `docs/analysis/use-case-fit-post-adr005.md`.

See [Release Gate Analysis](#release-gate-analysis) at the end of this document for
the explicit reasoning about what is a hard release requirement versus what can be
deferred to 0.5.0.

**Option E (implicit ledger)** has not been decided yet and is listed below as a
candidate under discussion — it will be confirmed or deferred before implementation begins.

The `ReflectionMessageHandler` compiled-delegate optimisation previously listed here has
been deferred to 0.5.0. See `docs/future-plans/0.5.0-roadmap.md`.
...
|---|---------|-----------|--------|
| 1 | [Cross-Process Append Safety (ADR-005)](#1-cross-process-append-safety-adr-005) | No | Medium |
| 2 | [Write Throughput Optimization (A+B, E under discussion)](#2-write-throughput-optimization-ab-e-under-discussion) | Yes (index format; ledger TBD) | Medium |

---

## 1. Cross-Process Append Safety (ADR-005)

Full decision record: `docs/decisions/005-cross-process-append-safety.md`
...
## 2. Write Throughput Optimization (A+B, E under discussion)
...
|------|-------------------|--------|
| ADR-005 | Correctness (not speed) | ✅ Confirmed |
| Option A | ~92 → ~105/sec (+14%) | ✅ Confirmed |
| Option B | ~105 → ~125/sec (+19%) | ✅ Confirmed |
| Option E | ~125 → ~185/sec (+48%) | ⚠️ Under discussion |

### Breaking impact summary

| Change | Breaking? | Migration | Status |
|--------|-----------|-----------|--------|
| ADR-005 | No | None | ✅ Confirmed |
| Option A (index format) | Yes | One-time migration on first startup | ✅ Confirmed |
| Option B (in-memory cache) | No | None | ✅ Confirmed |
| Option E (implicit ledger) | Yes | Transparent — startup scan re-derives position | ⚠️ Under discussion |
...
| Item | Release gate? | Rationale |
|------|---------------|-----------|
| ADR-005 (cross-process lock) | ✅ Hard requirement | Multi-user correctness — broken without it |
| Option A (append-only index format) | ⚠️ Opportunistic | Breaking format change; free to defer but see note below |
| Option B (in-memory index cache) | ❌ Deferrable | Pure optimisation; zero format impact; ships any time |
| Option E (implicit ledger) | ❌ Deferrable | Under discussion; no correctness or throughput gate |
...
### Recommended 0.5.0 deferral candidates

| Item | Defer to 0.5.0? | Reason |
|------|----------------|--------|
| Option B (in-memory index cache) | ✅ Safe to defer | No format impact; pure runtime optimisation |
| Option E (implicit ledger) | ✅ Safe to defer | Decision still pending; not a gate |
| `ReflectionMessageHandler` delegate cache | ✅ Already deferred | See `docs/future-plans/0.5.0-roadmap.md` |
