...
All three are **API-level changes** — each one produces at least one breaking change — which
...
|---|---------|-----------|--------|
| 1 | [Multi-context support](#1-multi-context-support) | Yes — new parameter on `ReadAsync` / `AppendAsync` | Large |
| 2 | [Streaming reads via `IAsyncEnumerable<SequencedEvent>`](#2-streaming-reads-via-iasyncenumerablesequencedevent) | Yes — `IEventStore.ReadAsync` return type changes | Medium |
| 3 | [`Apply` receives `SequencedEvent` instead of `IEvent`](#3-apply-receives-sequencedevent-instead-of-ievent) | Yes — `IProjectionDefinition<TState>` signature changes | Small |
...
1. **`Apply(SequencedEvent)` first** — smallest change, entirely within the projection
...
2. **Streaming reads second** — changes `IEventStore` but not the storage format.
...
3. **Multi-context last** — builds on the stabilised `IEventStore` signature produced
...
|----------|------|--------------|
| 🔴 | Add `CancellationToken` to `AppendAsync(NewEvent[], AppendCondition?)` and `ReadAsync(Query, ReadOption[]?, long?)` | Breaking change — needs a version bump |
| 🟠 | Make `Tag` and `QueryItem` immutable `record` types | Breaking change |
| 🟠 | Extract duplicated `TagIndex`/`EventTypeIndex` code to a shared utility | Refactor scope, risk of subtle regression |
| 🟡 | Metadata mutation in `AppendAsync(NewEvent[], AppendCondition?)` is surprising | Requires API design decision |
